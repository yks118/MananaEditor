if (typeof MananaEditor === 'function') {
    MananaEditor.registerPlugin('uploadVideo', function (objEditor, options) {
        this.options = objEditor.deepMerge({
            icon: '<button type="button"><i class="far fa-file-video fa-fw"></i></button>',
            mode: 'base64',
            accept: 'video/mp4,video/webm',
            text: 'Your browser does not support the video tag.',
            uploadURL: null,
            insertNode: {
                _videoTag: function (objEditor, objFile, objResponse) {
                    if (typeof objResponse.src === 'undefined' || !objResponse.src) {
                        console.log('Not Data: objResponse.src');
                        return false;
                    }

                    let objVideo = document.createElement('video');
                    objVideo.controls = true;
                    // objVideo.autoplay = true;
                    objVideo.style.maxWidth = '100%';
                    ...
                    let objText = document.createTextNode(objEditor.options.plugins['uploadVideo'].text);

                    objVideo.appendChild(objSource);
                    objVideo.appendChild(objText);

                    objEditor.insertNode(objVideo);
                ...
                server: async function (objEditor, objFile) {
                    if (objEditor.options.plugins['uploadVideo'].uploadURL === null) {
                        console.log('Not Data: objEditor.options.plugins[\'uploadVideo\'].uploadURL');
                        return false;
                    }
                    ...
                    return await fetch(objEditor.options.plugins['uploadVideo'].uploadURL, {
                        method: 'POST',
                        body: objFormData,
                    }).then((response) => {
                        if (response.ok) {
        ...
        this.options.iconElement.addEventListener('click', event => {
            ...
            objInputFile.addEventListener('change', function (event) {
                ...
                if (this.files && this.files.length > 0) {
                    (async () => {
                        let strMode = objEditor.options.plugins['uploadVideo'].mode;
                        for await (let i = 0; i < this.files.length; i++) {
                            let objResponse = await objEditor.options.plugins['uploadVideo'].insertNode[strMode](objEditor, this.files[i]);
                            if (typeof objResponse.src === 'string') {
                                objEditor.options.plugins['uploadVideo'].insertNode['_videoTag'](objEditor, this.files[i], objResponse);
                            }
                        }
